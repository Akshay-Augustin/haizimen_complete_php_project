...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    foreach ($days as $day) {
        ...
        $stmt = $conn->prepare("
            INSERT INTO doctor_availability (doctor_user_id, day_name, is_available, start_time, end_time)
            VALUES (?, ?, ?, ?, ?)
...
$availabilityMap = [];
$stmt = $conn->prepare("SELECT * FROM doctor_availability WHERE doctor_user_id = ?");
            ...
            <table>
                ...
                <?php foreach ($days as $day): ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="checkbox" name="is_available[<?php echo $key; ?>]" value="1"
                                <?php echo (!empty($row) && (int)$row['is_available'] === 1) ? 'checked' : ''; ?>>
                        </td>
                        <td>
                            <input type="time" name="start_time[<?php echo $key; ?>]"
                                value="<?php echo e($row['start_time'] ?? ''); ?>">
                        </td>
                        <td>
                            <input type="time" name="end_time[<?php echo $key; ?>]"
